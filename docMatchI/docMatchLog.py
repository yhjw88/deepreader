...
class LogisticRegression:
    ...
    def doWork(self, learningRate):
        ...
        while True:
            ...
            if i % 10000 == 0:
                ...
                sys.stdout.flush()
            if np.linalg.norm(prev_theta - theta) < 1e-7: # 1e-15:
    ...
    def plotLin(self, filename):
        ...
        linep2x2 = 0
        linep2x1 = -1 * (theta[0] + theta[2] * linep2x2) / theta[1]
...
def reg1():
    ...
    logisticRegression = LogisticRegression(trainX, trainY)
    logisticRegression.doWork(10)
    logisticRegression.plotLin("data/docMatchITrain.png")

    # Train accuracy
    predictedY = logisticRegression.getPredicted(trainX)
    print "Train correct: {} total: {}".format(countCorrect(trainY, predictedY), len(trainY))
